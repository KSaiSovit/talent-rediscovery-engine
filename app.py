...
# =============================
# HARD FILTER (with configurable threshold)
# =============================
def passes_filter(row, jd_struct, threshold=0.3):
    skills = tokenize(row.get("Skills", ""))
    must = jd_struct["must_have"]
    
    if not must:   # no must‑have skills → everyone passes
        return True
    
    match = sum(1 for m in must if any(m in s for s in skills))
    # require at least 1 match, and at least threshold fraction of must‑have skills
...
def main():
    ...
    if uploaded and jd:
        ...
        # --- Hard filter threshold slider ---
        threshold_pct = st.slider(
            "Minimum % of must‑have skills required to pass hard filter",
            min_value=0, max_value=100, value=30, step=5
        ) / 100.0
        
        must_count = len(jd_struct["must_have"])
        if must_count > 0:
            required = max(1, int(threshold_pct * must_count))
            st.caption(f"🔍 Requires at least **{required}** out of {must_count} must‑have skill(s).")
        else:
            st.caption("🔍 No must‑have skills detected – all candidates will pass the hard filter.")
        # ------------------------------------
        
        # Apply hard filter
        df = df[df.apply(lambda x: passes_filter(x, jd_struct, threshold_pct), axis=1)]
        
        # Check if any candidate remains
        if df.empty:
            st.warning("❌ No candidates passed the hard filter. Try lowering the threshold or check your CSV columns and job description.")
            st.stop()
